...
from src.f1predictor.championship_probability import calculate_constructor_title_probabilities, calculate_title_probabilities
...
from src.f1predictor.driver_animation import driver_event_summary, plot_driver_position_animation, plot_driver_race_story, plot_single_driver_track_animation
from src.f1predictor.driver_profile_dashboard import (
    build_all_driver_profiles,
    build_driver_profile,
    plot_all_driver_profile_matrix,
    plot_driver_skill_radar,
    plot_driver_status_animation,
)
...
from src.f1predictor.race_grid_simulator import plot_position_evolution, plot_race_animation, plot_starting_grid, simulate_lap_by_lap_order
...
from src.f1predictor.team_center import build_all_team_profiles, build_team_profile, plot_team_comparison, plot_team_race_positions, plot_team_radar
from src.f1predictor.telemetry.degradation_dashboard import detect_tire_cliff, load_tire_degradation, plot_degradation
from src.f1predictor.telemetry.streamlit_insights import compare_driver_controls, compare_driver_speed_trace, load_fastest_lap_telemetry
...
st.write("Unified Formula 1 forecasting, simulation, telemetry, race control, driver/team analytics, replay, broadcast, and diagnostics platform.")
...
def build_full_race_state(race_name, total_laps, pit_loss, rain_probability, seed):
    ...
    director_log = generate_race_director_log(timeline, random_state=int(seed) + 99)
    adjusted_final = apply_director_time_penalties(replay["final_order"], director_log)
    driver_profiles = build_all_driver_profiles(timeline)
...
def render_lap_panel(state, lap_number, total_laps):
    ...
    c4.metric("DRS/Overtakes", len(lap_alerts))
    c5.metric("Track Grip", f"{float(lap_weather.iloc[0]['TrackGrip']) * 100:.0f}%" if not lap_weather.empty else "N/A")

    live_timing, sectors_tab, radio_tab, broadcast_tab = st.tabs(["Leaderboard", "Sectors / DRS / Weather", "Team Radio", "TV Broadcast"])
    with live_timing:
        ...
        st.plotly_chart(px.bar(lap_df, x="Driver", y="GapToLeader", color="Compound", hover_data=cols, title=f"Gap to leader — Lap {lap_number}"), use_container_width=True)
    with sectors_tab:
        ...
        with left:
            ...
            if not lap_sectors.empty:
                st.plotly_chart(px.bar(lap_sectors.head(10), x="Driver", y=["S1", "S2", "S3"], title="Sector timing estimate"), use_container_width=True)
        with right:
            st.subheader("DRS / Overtake Center")
            st.dataframe(lap_alerts, use_container_width=True, hide_index=True) if not lap_alerts.empty else st.info("No DRS or overtake alerts this lap.")
...
if build_full:
    with st.spinner("Building forecast, replay, race director, timing, driver/team centers, radio and broadcast layer..."):
...
tabs = st.tabs([
    "Overview", "Forecasting", "Championship", "Live Race Control", "Race Replay",
    "Driver Center", "Team Center", "Race Director", "Telemetry", "Tyres", "FastF1 Training", "Diagnostics",
])
(
    overview_tab, forecasting_tab, championship_tab, live_tab, replay_tab, driver_tab,
    team_tab, director_tab, telemetry_tab, tyres_tab, training_tab, diagnostics_tab,
) = tabs
...
with overview_tab:
    ...
    c2.metric("Race Engine", "Lap-by-lap")
    c3.metric("Broadcast", "Radio + TV")
    c4.metric("Race Director", "Incidents + Penalties")
    c5.metric("Centers", "Driver + Team")
    if "unified_state" in st.session_state:
        ...
        st.subheader("Adjusted final classification")
        st.dataframe(state["adjusted_final"], use_container_width=True, hide_index=True)
    else:
...
with forecasting_tab:
    ...
    if st.button("Run standalone future forecast"):
        result = forecast_future_race(cached_demo_dataset(), race_name, 2026, 12, n_simulations=n_simulations, noise_std=noise_std, use_2026_grid=True)
        forecast, sim, weather = result["forecast"], result["simulation"], result["weather"]
...
with championship_tab:
    ...
    if st.button("Run championship simulation"):
        season = simulate_2026_season(cached_demo_dataset(), n_simulations=season_runs, noise_std=noise_std)
...
with live_tab:
    ...
    if "unified_state" not in st.session_state:
        ...
        if controls[0].button("⏮ Lap 1"):
            st.session_state["unified_lap"] = 1
        if controls[1].button("◀ Previous"):
            st.session_state["unified_lap"] = max(1, int(st.session_state.get("unified_lap", 1)) - 1)
        if controls[2].button("Next ▶"):
            st.session_state["unified_lap"] = min(total, int(st.session_state.get("unified_lap", 1)) + 1)
        st.session_state["unified_lap"] = controls[3].slider("Manual lap selector", 1, total, int(st.session_state.get("unified_lap", 1)))
        c1, c2 = st.columns([1, 1])
        autoplay = c1.button("▶ Play race")
        if c2.button("🏁 Finish"):
...
with driver_tab:
    ...
    if "unified_state" not in st.session_state:
        ...
        profile = build_driver_profile(selected_driver, state["timeline"])
        p1, p2, p3, p4 = st.columns(4)
        p1.metric("Start", profile.get("StartPosition", pd.Series(["N/A"])).iloc[0])
        p2.metric("Finish", profile.get("FinalPosition", pd.Series(["N/A"])).iloc[0])
        p3.metric("Best", profile.get("BestPosition", pd.Series(["N/A"])).iloc[0])
        p4.metric("Pit Stops", profile.get("PitStops", pd.Series(["N/A"])).iloc[0])
        st.dataframe(profile, use_container_width=True, hide_index=True)
        st.plotly_chart(plot_driver_skill_radar(selected_driver), use_container_width=True)
        st.plotly_chart(plot_driver_status_animation(state["timeline"], selected_driver), use_container_width=True)
        ...
        st.subheader("All driver matrix")
        st.plotly_chart(plot_all_driver_profile_matrix(state["driver_profiles"]), use_container_width=True)
        st.dataframe(state["driver_profiles"], use_container_width=True, hide_index=True)
        st.subheader("Driver event summary")
        st.dataframe(driver_event_summary(state["timeline"], state["pitstops"], selected_driver), use_container_width=True, hide_index=True)

with team_tab:
    st.subheader("Team Center")
    if "unified_state" not in st.session_state:
        st.info("Build the unified race state from the sidebar first.")
    else:
        state = st.session_state["unified_state"]
        teams = sorted(state["timeline"]["Team"].dropna().unique())
        selected_team = st.selectbox("Team", teams)
        team_profile = build_team_profile(selected_team, state["timeline"])
        ...
        st.plotly_chart(plot_team_race_positions(state["timeline"], selected_team), use_container_width=True)
        st.subheader("All team matrix")
        st.plotly_chart(plot_team_comparison(state["team_profiles"]), use_container_width=True)
        st.dataframe(state["team_profiles"], use_container_width=True, hide_index=True)

with director_tab:
    st.subheader("Race Director Center")
    if "unified_state" not in st.session_state:
        st.info("Build the unified race state from the sidebar first.")
    else:
        state = st.session_state["unified_state"]
        ...
        st.subheader("Steward summary")
        st.dataframe(summary, use_container_width=True, hide_index=True)
        st.plotly_chart(plot_race_director_timeline(director_log), use_container_width=True)
        st.subheader("Incident log")
        st.dataframe(director_log, use_container_width=True, hide_index=True)
        st.subheader("Classification after penalties")
        st.dataframe(state["adjusted_final"], use_container_width=True, hide_index=True)
